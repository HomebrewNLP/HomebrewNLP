...
def conv(inp: torch.Tensor, weight: torch.Tensor, groups: int) -> torch.Tensor:
    ...
    return torch.nn.functional.conv1d(inp, weight, groups=groups)   # type: ignore
...
class LinearAttentionCell(torch.nn.Module):
    ...
    def forward(self, inp: torch.Tensor) -> torch.Tensor:
        ...
        cum, out = linear_attention(depth, scale, shift, divisor, self.init_scale, self._cumsum_cache)   # type: ignore
